...
- FEEDBACK: What does "mutually recursive" mean? This currently could be interpreted as "derivations can only depend on derived references and derived references only on derivations".
  ...
  RESPONSE: Well, it's not that they *have* to depend on each other.
  If that were the case, then there would be no base case!
  It's that each *can* depend on the other.
  There are *two* base cases in fact:
   - derivation with no inputs (though it will fail unless sandboxing is bypassed)
   - opaque derived path
...
**Built reference**
A built reference is a pair of a derivation reference and an output name.
