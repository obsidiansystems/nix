...
StringMap EvalState::realiseContext(const PathSet &context, const Pos &pos,
  ...
  for (auto &i : context) {
    ...
    if (!outputName.empty() && ctx.isDerivation()) {
      ...
      if (evalSettings.logAllIFD) {
        printInfo("%1% importing from derivation %2% via %3%", pos, ctxS,
                  reason);
      }
    } else {
  ...
  if (!evalSettings.enableImportFromDerivation)
    throw Error("cannot build '%1%' during evaluation because the option "
  ...
  for (auto &[drvPath, outputs] : drvs) {
    ...
    for (auto &outputName : outputs) {
      ...
      res.insert_or_assign(downstreamPlaceholder(*store, drvPath, outputName),
...
static void mkOutputString(EvalState &state, BindingsBuilder &attrs,
  ...
  attrs.alloc(o.first).mkString(
      optOutputPath ? state.store->printStorePath(*optOutputPath)
                    /* Downstream we would substitute this for an actual path
                       once we build the floating CA derivation */
...
/* Load and evaluate an expression from path specified by the
   argument. */
static void import(EvalState &state, const Pos &pos, Value &vPath,
...
static RegisterPrimOp primop_import({.name = "import",
                                     .args = {"path"},
                                     .doc = R"(
      ...
      *path* is a directory, the file ` default.nix ` in that directory
      ...
      any name would work.)
                                     ...
                                     .fun = [](EvalState &state, const Pos &pos,
                                               Value **args, Value &v) {
...
void prim_importNative(EvalState &state, const Pos &pos, Value **args,
  ...
  if (!func) {
    ...
    if (message)
      throw EvalError("could not load symbol '%1%' from '%2%': %3%", sym, path,
                      message);
    else
      throw EvalError("symbol '%1%' from '%2%' resolved to NULL when a "
...
void prim_exec(EvalState &state, const Pos &pos, Value **args, Value &v) {
  ...
  if (count == 0) {
    throw EvalError({.msg = hintfmt("at least one argument to 'exec' required"),
                     .errPos = pos});
  }
  PathSet context;
  auto program =
      state.coerceToString(pos, *elems[0], context, false, false).toOwned();
  Strings commandArgs;
  for (unsigned int i = 1; i < args[0]->listSize(); ++i) {
    commandArgs.push_back(
        state.coerceToString(pos, *elems[i], context, false, false).toOwned());
  }
  try {
    auto _ = state.realiseContext(context, pos,
                                  "exec"); // FIXME: Handle CA derivations
...
struct CompareValues {
  ...
  bool operator()(Value *v1, Value *v2) const {
    ...
    if (v1->type() != v2->type())
      throw EvalError("cannot compare %1% with %2%", showType(*v1),
                      showType(*v2));
    switch (v1->type()) {
    case nInt:
...
static Bindings::iterator getAttr(EvalState &state, std::string_view funcName,
                                  Symbol attrSym, Bindings *attrSet,
                                  const Pos &pos) {
  Bindings::iterator value = attrSet->find(attrSym);
  if (value == attrSet->end()) {
    hintformat errorMsg =
        hintfmt("attribute '%s' missing for call to '%s'", attrSym, funcName);
...
static void prim_genericClosure(EvalState &state, const Pos &pos, Value **args,
  ...
  Bindings::iterator startSet =
      getAttr(state, "genericClosure", state.sStartSet, args[0]->attrs, pos);
  ...
  Bindings::iterator op =
      getAttr(state, "genericClosure", state.sOperator, args[0]->attrs, pos);
  ...
  while (!workSet.empty()) {
    ...
    if (key == e->attrs->end())
      throw EvalError(
          {.msg = hintfmt("attribute 'key' required"), .errPos = pos});
...
static RegisterPrimOp primop_abort(
    {.name = "abort",
     ...
     .fun = [](EvalState &state, const Pos &pos, Value **args, Value &v) {
       ...
       throw Abort("evaluation aborted with the following error message: '%1%'",
                   s);
     }});
...
static void prim_addErrorContext(EvalState &state, const Pos &pos, Value **args,
  ...
  try {
    ...
    PathSet context;
    e.addTrace(std::nullopt,
               state.coerceToString(pos, *args[0], context).toOwned());
    throw;
  }
}
...
static void prim_ceil(EvalState &state, const Pos &pos, Value **args,
                      Value &v) {
...
static void prim_floor(EvalState &state, const Pos &pos, Value **args,
                       Value &v) {
  auto value = state.forceFloat(*args[0], args[0]->determinePos(pos));
  v.mkInt(floor(value));
}
...
static void prim_derivationStrict(EvalState &state, const Pos &pos,
  ...
  Bindings::iterator attr =
      getAttr(state, "derivationStrict", state.sName, args[0]->attrs, pos);
    ...
    try {
      ...
      /* The `args' attribute is special: it supplies the
        ...
        for (auto elem : i->value->listItems()) {
          auto s =
              state.coerceToString(posDrvName, *elem, context, true).toOwned();
      ...
      /* All other attributes are passed to the builder through
        ...
        if (jsonObject) {
          ...
          auto s =
              state.coerceToString(*i->pos, *i->value, context, true).toOwned();
  ...
  for (auto &path : context) {
    ...
    if (path.at(0) == '=') {
      ...
      StorePathSet refs;
      state.store->computeFSClosure(
          state.store->parseStorePath(std::string_view(path).substr(1)), refs);
      for (auto &j : refs) {
  ...
  if (drv.builder == "")
    throw EvalError({.msg = hintfmt("required attribute 'builder' missing"),
                     .errPos = posDrvName});

  if (drv.platform == "")
    throw EvalError({.msg = hintfmt("required attribute 'system' missing"),
                     .errPos = posDrvName});

  /* Check whether the derivation name is valid. */
  if (isDerivation(drvName))
    throw EvalError(
        {.msg = hintfmt("derivation names are not allowed to end in '%s'",
                        drvExtension),
         .errPos = posDrvName});
  ...
  if (outputHash) {
    ...
    if (outputs.size() != 1 || *(outputs.begin()) != "out")
      throw Error({.msg = hintfmt("multiple outputs are not supported in "
    ...
    drv.outputs.insert_or_assign("out", DerivationOutput::CAFixed{
                                            .hash =
                                                FixedOutputHash{
                                                    .method = method,
                                                    .hash = std::move(h),
...
static void prim_storePath(EvalState &state, const Pos &pos, Value **args,
  ...
  if (evalSettings.pureEval)
    throw EvalError(
        {.msg = hintfmt("'%s' is not allowed in pure evaluation mode",
                        "builtins.storePath"),
         .errPos = pos});
  ...
  if (!state.store->isInStore(path))
    throw EvalError({.msg = hintfmt("path '%1%' is not in the Nix store", path),
                     .errPos = pos});
...
static void prim_pathExists(EvalState &state, const Pos &pos, Value **args,
  ...
  auto path = realisePath(state, pos, *args[0], "pathExists",
                          {.checkForPureEval = false});
...
static void prim_baseNameOf(EvalState &state, const Pos &pos, Value **args,
  ...
  PathSet context;
  v.mkString(
      baseNameOf(*state.coerceToString(pos, *args[0], context, false, false)),
      context);
}
...
static void prim_readFile(EvalState &state, const Pos &pos, Value **args,
  ...
  if (state.store->isInStore(path)) {
    try {
      refs = state.store->queryPathInfo(state.store->toStorePath(path).first)
                 ->references;
...
static void prim_findFile(EvalState &state, const Pos &pos, Value **args,
  ...
  for (auto v2 : args[0]->listItems()) {
    ...
    PathSet context;
    auto path =
        state.coerceToString(pos, *i->value, context, false, false).toOwned();
...
static void prim_hashFile(EvalState &state, const Pos &pos, Value **args,
  ...
  if (!ht)
    throw Error(
        {.msg = hintfmt("unknown hash type '%1%'", type), .errPos = pos});
...
static void prim_readDir(EvalState &state, const Pos &pos, Value **args,
  ...
  for (auto &ent : entries) {
    ...
    attrs.alloc(ent.name).mkString(ent.type == DT_REG   ? "regular"
...
static void addPath(EvalState &state, const Pos &pos, const std::string &name,
  ...
  try {
    ...
    path = evalSettings.pureEval && expectedHash ? path
    ...
    if (expectedHash)
      expectedStorePath =
          state.store->makeFixedOutputPath(method, *expectedHash, name);
...
static void prim_filterSource(EvalState &state, const Pos &pos, Value **args,
  ...
  if (args[0]->type() != nFunction)
    throw TypeError({.msg = hintfmt("first argument in call to 'filterSource' "
                                    "is not a function but %1%",
                                    showType(*args[0])),
                     .errPos = pos});

  addPath(state, pos, std::string(baseNameOf(path)), path, args[0],
...
static void prim_path(EvalState &state, const Pos &pos, Value **args,
  ...
  for (auto &attr : *args[0]->attrs) {
    ...
    if (n == "path")
      ...
      expectedHash = newHashAllowEmpty(
          state.forceStringNoCtx(*attr.value, *attr.pos), htSHA256);
    else
      throw EvalError(
  ...
  if (path.empty())
    throw EvalError({.msg = hintfmt("'path' required"), .errPos = pos});
  if (name.empty())
...
void prim_getAttr(EvalState &state, const Pos &pos, Value **args, Value &v) {
  ...
  Bindings::iterator i = getAttr(state, "getAttr", state.symbols.create(attr),
                                 args[1]->attrs, pos);
  // !!! add to stack trace?
  if (state.countCalls && *i->pos != noPos)
...
static void prim_unsafeGetAttrPos(EvalState &state, const Pos &pos,
                                  Value **args, Value &v) {
  auto attr = state.forceStringNoCtx(*args[0], pos);
  state.forceAttrs(*args[1], pos);
...
static void prim_listToAttrs(EvalState &state, const Pos &pos, Value **args,
  ...
  for (auto v2 : args[0]->listItems()) {
    ...
    Bindings::iterator j =
        getAttr(state, "listToAttrs", state.sName, v2->attrs, pos);
    ...
    if (seen.insert(sym).second) {
      Bindings::iterator j2 =
          getAttr(state, "listToAttrs", state.sValue, v2->attrs, pos);
...
static void prim_intersectAttrs(EvalState &state, const Pos &pos, Value **args,
  ...
  auto attrs = state.buildBindings(
      std::min(args[0]->attrs->size(), args[1]->attrs->size()));
...
static void prim_functionArgs(EvalState &state, const Pos &pos, Value **args,
  ...
  if (!args[0]->isLambda())
    throw TypeError(
        {.msg = hintfmt("'functionArgs' requires a function"), .errPos = pos});
...
static void prim_zipAttrsWith(EvalState &state, const Pos &pos, Value **args,
                              Value &v) {
...
static void elemAt(EvalState &state, const Pos &pos, Value &list, int n,
  ...
  if (n < 0 || (unsigned int)n >= list.listSize())
    throw Error(
        {.msg = hintfmt("list index %1% is out of bounds", n), .errPos = pos});
...
static void prim_tail(EvalState &state, const Pos &pos, Value **args,
  ...
  if (args[0]->listSize() == 0)
    throw Error(
        {.msg = hintfmt("'tail' called on an empty list"), .errPos = pos});
...
static void prim_genList(EvalState &state, const Pos &pos, Value **args,
  ...
  if (len < 0)
    throw EvalError(
        {.msg = hintfmt("cannot create list of size %1%", len), .errPos = pos});
...
static void prim_groupBy(EvalState &state, const Pos &pos, Value **args,
                         Value &v) {
  state.forceFunction(*args[0], pos);
  state.forceList(*args[1], pos);
...
static void prim_add(EvalState &state, const Pos &pos, Value **args, Value &v) {
  ...
  if (args[0]->type() == nFloat || args[1]->type() == nFloat)
    v.mkFloat(state.forceFloat(*args[0], pos) +
...
static void prim_div(EvalState &state, const Pos &pos, Value **args, Value &v) {
  ...
  if (f2 == 0)
    throw EvalError({.msg = hintfmt("division by zero"), .errPos = pos});
  ...
  if (args[0]->type() == nFloat || args[1]->type() == nFloat) {
    ...
    if (i1 == std::numeric_limits<NixInt>::min() && i2 == -1)
      throw EvalError(
          {.msg = hintfmt("overflow in integer division"), .errPos = pos});
...
static void prim_substring(EvalState &state, const Pos &pos, Value **args,
  ...
  if (start < 0)
    throw EvalError({.msg = hintfmt("negative start position in 'substring'"),
                     .errPos = pos});
...
void prim_match(EvalState &state, const Pos &pos, Value **args, Value &v) {
  ...
  try {
    ...
    if (e.code() == std::regex_constants::error_space) {
      ...
      throw EvalError({.msg = hintfmt("invalid regular expression '%s'", re),
                       .errPos = pos});
    }
  }
}
...
static void prim_concatStringsSep(EvalState &state, const Pos &pos,
                                  Value **args, Value &v) {
  PathSet context;
...
static void prim_replaceStrings(EvalState &state, const Pos &pos, Value **args,
  ...
  if (args[0]->listSize() != args[1]->listSize())
    throw EvalError({.msg = hintfmt("'from' and 'to' arguments to "
                                    "'replaceStrings' have different lengths"),
                     .errPos = pos});
...
#include "primops/derivation.nix.gen.hh"
      ...
      "\0";
  eval(parse(code, sizeof(code), foFile, sDerivationNix, "/", staticBaseEnv),
       *vDerivation);
}
