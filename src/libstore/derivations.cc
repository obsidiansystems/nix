    ...
    std::string_view drvName)
{
            ...
            auto modulo = [&] {
                if constexpr (std::is_same_v<std::decay_t<decltype(drv)>, derivation::Basic>)
                    /* A resolved derivation has no input derivations to
                       ...
                       lookup. */
                    return std::optional{derivation::masked::fullyMaskedDerivation(store, drv)};
                else
                    return derivation::masked::fullyMaskedDerivation(store, readDerivation, drv);
            }();
    ...
    /* Throws on invalid output combinations. Must run before
       `fullyMaskedDerivation`, which would panic instead. */
