 ...
 * paths; `fullyMaskedDerivation` wants both.
 *
        ...
        if constexpr (std::is_same_v<Out, Output>)
            std::visit(
                overloaded{
                    ...
                    [&](const auto &) {
                        panic(
                            "fullyMaskedDerivation: unexpected output type, these derivation types are not input addressed");
                    },
                },
                output.raw);
        return {};
...
template<typename Out>
std::optional<Drv<Output::Deferred>> fullyMaskedDerivation(
    const StoreDirConfig & store,
    ReadDerivation readDerivation,
...
fullyMaskedDerivation(const StoreDirConfig & store, ReadDerivation readDerivation, const Full & drv);
template std::optional<Drv<Output::Deferred>>
fullyMaskedDerivation(const StoreDirConfig & store, ReadDerivation readDerivation, const FullDeferred & drv);
template std::optional<Drv<Output::Deferred>>
fullyMaskedDerivation(const StoreDirConfig & store, ReadDerivation readDerivation, const FullInputAddressed & drv);

Drv<Output::Deferred> fullyMaskedDerivation(const StoreDirConfig & store, const Basic & drv)
{
