...
StorePath Store::makeFixedOutputPathFromCA(const StorePathDescriptor & desc) const
{
    ...
    return std::visit(overloaded {
        ...
        [&](IPFSInfo io) {
            return makeIPFSPath(desc.name, computeIPFSHash(desc));
        },
        [&](IPFSHash ic) {
            return makeIPFSPath(desc.name, ic);
        }
    }, desc.info);
}
    ...
    processGraph<OwnedStorePathOrDesc>(pool,
        ...
        [&](const OwnedStorePathOrDesc & storePathOrDesc) -> std::set<OwnedStorePathOrDesc> {
            ...
            if (auto storePathP = std::get_if<StorePath>(&storePathOrDesc)) {
                if (auto descOpt = info->fullStorePathDescriptorOpt()) {
                    auto & desc = *descOpt;
                    if (!std::holds_alternative<IPFSHash>(desc.info)) {
                        /* We cannot upgrade if we have an IPFSHash, because it
                           doesn't directly have the references as part of the
                           CA. */
        ...
        [&](const OwnedStorePathOrDesc & storePathOrDesc) {
            ...
            if (!dstStore->isValidPath(storePathOrDescB)) {
                ...
                showProgress();
                try {
                    if (dstStore->isTrusting || *info->viewCAConst()) {
                        ...
                        dstStore->ensurePath(storePathOrDescB);
                    }
                } catch (Error &e) {
                    nrFailed++;
