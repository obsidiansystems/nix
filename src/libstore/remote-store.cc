...
bool RemoteStore::isValidPathUncached(const StorePath & path, std::optional<FullContentAddress> ca)
{
...
void RemoteStore::ensurePath(const StorePath & path, std::optional<FullContentAddress> ca)
{
    auto conn(getConnection());
