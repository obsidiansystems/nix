...
TEST_F(MerkleTarAdapterTest, single_file_at_root)
{
    ...
    tarSink->createRegularFile(CanonPath::root, false, [](Sink & sink) {
        sink("hello world");
    });
...
TEST_F(MerkleTarAdapterTest, single_executable_file)
{
    ...
    tarSink->createRegularFile(CanonPath("script.sh"), true, [](Sink & sink) {
        sink("#!/bin/bash\necho hello");
    });
...
TEST_F(MerkleTarAdapterTest, directory_with_files)
{
    ...
    tarSink->createRegularFile(CanonPath("hello.txt"), false, [](Sink & sink) {
        sink("hello world");
    });
    tarSink->createRegularFile(CanonPath("bye.txt"), false, [](Sink & sink) {
        sink("goodbye");
    });
...
TEST_F(MerkleTarAdapterTest, nested_directory_with_files)
{
    ...
    tarSink->createRegularFile(CanonPath("a/b/file.txt"), false, [](Sink & sink) {
        sink("nested content");
    });
...
TEST_F(MerkleTarAdapterTest, mixed_content)
{
    ...
    tarSink->createRegularFile(CanonPath("file.txt"), false, [](Sink & sink) {
        sink("regular file");
    });
    tarSink->createRegularFile(CanonPath("script.sh"), true, [](Sink & sink) {
        sink("#!/bin/bash");
    ...
    tarSink->createRegularFile(CanonPath("subdir/nested.txt"), false, [](Sink & sink) {
        sink("nested");
    });
...
TEST_F(MerkleTarAdapterTest, hardlink_target_not_found)
{
    ...
    try {
        ...
        ASSERT_THAT(e.msg(), testing::HasSubstr("not found"));
    }
}
...
TEST_F(MerkleTarAdapterTest, hardlink_to_file)
{
    ...
    tarSink->createRegularFile(CanonPath("original.txt"), false, [](Sink & sink) {
        sink("shared content");
    });
...
TEST_F(MerkleTarAdapterTest, multiple_hardlinks_same_target)
{
    ...
    tarSink->createRegularFile(CanonPath("original"), false, [](Sink & sink) {
        sink("content");
    });
...
TEST_F(MerkleTarAdapterTest, hardlink_to_directory_fails)
{
    ...
    try {
        ...
        ASSERT_THAT(e.msg(), testing::HasSubstr("directory"));
    }
}
...
TEST_F(MerkleTarAdapterTest, many_files)
{
    ...
    for (int i = 0; i < numFiles; i++) {
        ...
        tarSink->createRegularFile(path, false, [&](Sink & sink) {
            sink(content);
        });
    }
...
TEST_F(MerkleTarAdapterTest, deep_nesting)
{
    ...
    tarSink->createRegularFile(CanonPath("a/b/c/d/e/f/g/h/file.txt"), false, [](Sink & sink) {
        sink("deeply nested");
    });
...
TEST_F(MerkleTarAdapterTest, last_wins_file_overwrites_symlink)
{
    ...
    tarSink->createRegularFile(CanonPath("entry"), false, [](Sink & sink) {
        sink("file content");
    });
...
TEST_F(MerkleTarAdapterTest, last_wins_executable_changes)
{
    ...
    tarSink->createRegularFile(CanonPath("script"), false, [](Sink & sink) {
        sink("content");
    });
    tarSink->createRegularFile(CanonPath("script"), true, [](Sink & sink) {
        sink("content");
    });
...
RC_GTEST_FIXTURE_PROP(MerkleTarAdapterTest, last_wins_property, ())
{
    ...
    for (auto & [path, content] : entries) {
        tarSink->createRegularFile(CanonPath(path), false, [&](Sink & sink) {
            sink(content);
        });
