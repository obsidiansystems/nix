...
{
    auto childPath = dirPath / std::string{name};
    RestoreSink childSink{back.startFsync};
    childSink.dstPath = childPath;
...
void RestoreSink::createDirectory(DirectoryCreatedCallback callback)
{
    ...
    if (dirFd != INVALID_DESCRIPTOR) {
        if (::mkdirat(dirFd, dstPath.filename().c_str(), 0777) == -1)
    ...
    if (dirFd != INVALID_DESCRIPTOR) {
        newDirFd =
            AutoCloseFD{::openat(dirFd, dstPath.filename().c_str(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC)};
    } else {
...
void RestoreSink::createRegularFile(bool isExecutable, RegularFileCreatedCallback func)
{
    auto crf = RestoreRegularFile(
        ...
        [&]() {
            ...
            return AutoCloseFD{::openat(dirFd, dstPath.filename().c_str(), flags, 0666)};
        }()
#endif
    );
    if (!crf.fd)
...
#ifndef _WIN32
    if (dirFd != INVALID_DESCRIPTOR) {
        if (::symlinkat(requireCString(target), dirFd, dstPath.filename().c_str()) == -1)
