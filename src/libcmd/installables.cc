...
// boilerplat to get upcast to work

struct InstallableStorePath: public virtual Installable, public CoreInstallableStorePath
{
    InstallableStorePath(CoreInstallableStorePath && i)
        : Installable()
        , CoreInstallableStorePath(i)
    { }
};
...
static void fauxUpcastInstallabeVec(
    std::vector<std::shared_ptr<CoreInstallable>> & coreInstallables,
    const std::vector<std::shared_ptr<Installable>> & installables)
{
    assert(coreInstallables.empty());
    for (const auto & i : installables)
        coreInstallables.push_back(
            static_cast<std::shared_ptr<CoreInstallable>>(i));
}
...
void InstallablesCommand::prepare()
{
    ...
    fauxUpcastInstallabeVec(coreInstallables, installables);
}

std::vector<std::shared_ptr<CoreInstallable>> SourceExprCommand::parseCoreInstallables(
    ref<nix::Store> store, std::vector<std::string> _installables)
{
    std::vector<std::shared_ptr<CoreInstallable>> ret;
    auto installables = parseInstallables(store, _installables);
    fauxUpcastInstallabeVec(ret, installables);
    return ret;
}
